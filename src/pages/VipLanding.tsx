...
const useVipCalendarId = (profile: { user_id: string; vip_calendar_id: string | null } | null | undefined) =>
  useQuery({
    queryKey: ["vip_calendar_id", profile?.user_id],
    queryFn: async () => {
      if (!profile) return null;
      if (profile.vip_calendar_id) return profile.vip_calendar_id;
...
const VipLanding = () => {
  ...
  const { data: vipCalendarId } = useVipCalendarId(adminProfile);
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div id="agendar" className="relative animate-in fade-in slide-in-from-right-8 duration-1000 delay-300 scroll-mt-20">
              ...
              <div className="bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden p-6">
                {vipCalendarId ? (
                  <CalendarRenderer calendarId={vipCalendarId} />
                ) : (
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                {[T.why.check1, T.why.check2].map((c) => (
                  <div key={c} className="flex items-center gap-3 p-4 bg-background rounded-2xl border border-border/50 shadow-sm">
