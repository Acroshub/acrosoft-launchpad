...
import { Phone, MapPin, ChevronDown, ChevronLeft, ChevronRight, Shield, Clock, Leaf, Home, Star, CheckCheck, Sparkles, Wind, Check, Dog } from "lucide-react";
...
const SERVICES = [
  ...
  { icon: Dog,      title: "Pet-Friendly Cleaning",   subtitle: "Safe for pets & kids",           desc: "We use non-toxic, pet-safe products throughout. Special attention to pet hair, dander, and odor — so your home smells fresh even with furry family members.", photo: "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=800&q=80" },
...
const FAQS = [
  { q: "How is pricing determined?", a: "Our pricing is based on the size of your home (number of bedrooms and bathrooms) and whether you have pets. Book directly through our calendar above, or call us at (214) 295-0378 for a free quote — straight price, no hidden fees." },
  ...
  { q: "Do you offer recurring service discounts?", a: "Yes. Clients on weekly or bi-weekly plans receive a reduced rate. Ask about our recurring packages when you book." },
];
...
function BookingCalendar() {
  ...
  const inputCls = "w-full border border-slate-200 focus:border-sky-500 rounded-lg px-3 py-2.5 text-base md:text-sm text-slate-900 placeholder-slate-300 outline-none transition-colors";

  if (submitted) {
    return (
      <div className="min-h-[420px] flex flex-col items-center justify-center text-center">
        <div className="w-14 h-14 bg-sky-500 rounded-full flex items-center justify-center mb-4 shadow-lg">
          <Check size={24} className="text-white" />
        </div>
        <h4 className="text-slate-900 font-black text-xl mcs-h">Booking Confirmed!</h4>
        <p className="text-slate-500 text-sm mt-2 max-w-xs leading-relaxed">
          We'll be at your home on{" "}
          <span className="font-bold text-slate-900">{MONTH_NAMES[month]} {selectedDay}</span> at{" "}
          <span className="font-bold text-slate-900">{selectedSlot}</span>.
        </p>
        <div className="mt-5 p-4 bg-sky-50 border border-sky-200 rounded-xl text-sm text-slate-500 max-w-xs">
          We'll reach out to <span className="font-bold text-slate-900">{phone}</span> to confirm your appointment.
        </div>
      </div>
    );
  }
  ...
  if (step === 2) {
    return (
      <div className="min-h-[420px] flex flex-col">
        <div className="mb-5 p-3.5 bg-sky-50 border border-sky-200 rounded-xl flex items-center justify-between gap-3">
          <div>
            <div className="text-[9px] font-black tracking-[2px] uppercase text-slate-400 mcs-h mb-0.5">Your Appointment</div>
            <div className="text-slate-900 font-black text-sm mcs-h">{MONTH_NAMES[month]} {selectedDay} · {selectedSlot}</div>
          </div>
          <button onClick={() => setStep(1)} className="text-sky-500 text-xs font-bold hover:underline cursor-pointer mcs-h shrink-0">
            ← Change
          </button>
        </div>

        <p className="text-[9px] font-black tracking-[2px] uppercase text-slate-400 mb-3 mcs-h">Your Contact Info</p>
        ...
        <button
          ...
          className="mt-5 w-full bg-sky-500 text-white font-black py-3 rounded-xl text-sm transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-sky-600 hover:scale-[1.02] mcs-h cursor-pointer">
          Confirm Booking →
        </button>
      </div>
    );
    ...
    <div className="min-h-[420px] flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <button onClick={prevMonth} disabled={isCurrentMonth}
          className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-100 disabled:opacity-25 disabled:cursor-not-allowed transition-colors">
          <ChevronLeft size={16} />
        </button>
        <span className="font-black text-slate-900 text-sm mcs-h tracking-wide">{MONTH_NAMES[month]} {year}</span>
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAY_LABELS.map(d => (
          <div key={d} className="text-center text-[9px] font-black text-sky-300 tracking-widest uppercase py-0.5">{d}</div>
        ))}
      </div>
          ...
          return (
            <button key={day} disabled={past || booked}
              ...
              className={`aspect-square rounded-lg text-sm font-semibold transition-all duration-150
                ${sel                           ? "bg-sky-500 text-white font-black shadow-md cursor-pointer"           : ""}
                ${tod && !sel                   ? "border-2 border-sky-500 text-sky-500 font-black cursor-pointer"      : ""}
                ${!past && !booked && !sel && !tod ? "text-slate-800 hover:bg-sky-50 cursor-pointer"                    : ""}
                ${past                          ? "text-slate-200 cursor-not-allowed"                                   : ""}
                ${booked && !past               ? "text-slate-300 line-through cursor-not-allowed"                      : ""}
              `}>
              {day}
            </button>
          );
      ...
      <div className="mt-3 pt-3 border-t border-slate-200">
        <p className="text-[9px] font-black tracking-[2px] uppercase text-slate-400 mb-2 mcs-h">
            ...
            return (
              <button key={slot} disabled={disabled}
                ...
                className={`px-1 py-2 rounded-lg text-xs font-semibold border transition-all duration-150
                  ${selSlot              ? "bg-sky-500 border-sky-500 text-white font-black cursor-pointer"                     : ""}
                  ${!disabled && !selSlot ? "border-slate-200 text-slate-600 hover:border-sky-500 hover:text-sky-500 cursor-pointer" : ""}
      ...
      <button
        ...
        className="mt-4 w-full bg-[#0A2540] text-white font-black py-3 rounded-xl text-sm transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed enabled:hover:bg-[#0f3060] enabled:hover:scale-[1.02] mcs-h cursor-pointer">
        Continue →
      </button>
    </div>
  );
...
// ── PAGE ──────────────────────────────────────────────────────────────────────
export default function MartinezCleaning2() {
    ...
    <div className="overflow-x-hidden bg-white" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="relative w-full z-50 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-[68px] flex items-center justify-between gap-4">
          ...
          <div className="hidden lg:flex items-center gap-7">
            {[["Services","#services"],["Why Us","#why-us"],["Coverage","#areas"],["Book Now","#booking"]].map(([l,h])=>(
              <a key={l} href={h} className="text-slate-500 hover:text-slate-900 text-sm font-medium transition-colors cursor-pointer">{l}</a>
            ))}
          </div>
          <a href="#booking" className="hidden md:flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white font-semibold px-4 py-2.5 rounded-lg text-sm transition-all duration-200 cursor-pointer shadow-sm hover:shadow-[0_4px_12px_rgba(14,165,233,0.4)]">
            Book Free Cleaning
          </a>
        </div>
      </nav>
      ...
      <section className="relative bg-white overflow-hidden">
        ...
        <img
          ...
          loading="eager"
        />
        <div className="hidden lg:block absolute inset-0 bg-white/[0.88] pointer-events-none"/>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 sm:px-8 pt-8 lg:pt-20 pb-0">
          <div className="grid lg:grid-cols-[1fr_420px] gap-10 xl:gap-16 items-start">
            ...
            <div className="pb-8 lg:pb-24">
              ...
              <h1 className="hidden lg:block text-[clamp(2.4rem,6vw,4.4rem)] font-black text-slate-900 leading-[1.0] tracking-[-0.025em] mb-6 mcs-h">
                Your home,<br />
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-8 lg:mb-10">
                <a href="#booking" className="w-full sm:w-auto inline-flex flex-col items-center justify-center gap-0.5 bg-sky-500 hover:bg-sky-600 text-white px-8 py-4 rounded-xl font-semibold text-base transition-all duration-200 cursor-pointer shadow-md hover:shadow-[0_6px_22px_rgba(14,165,233,0.4)] hover:scale-[1.02] mcs-h">
                  <span className="font-bold text-lg">Book Free Cleaning</span>
                  <span className="text-white/70 text-sm">Free · No Commitment</span>
                </a>
              ...
              {/* Trust badges */}
              <div className="flex flex-wrap gap-3 mb-10 lg:mb-0">
            ...
            <div id="booking" className="relative">
              <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
                <div className="bg-[#0A2540] px-6 pt-6 pb-5 border-b border-slate-200/20">
                  <span className="text-sky-400 text-[10px] font-black tracking-[3px] uppercase mcs-h">Free · No Commitment</span>
                  <h3 className="text-white font-black text-xl mt-1.5 mcs-h leading-tight">Book Your Free Cleaning</h3>
                  <p className="text-white/60 text-sm mt-1.5 leading-relaxed">Pick a date and time. We confirm within hours.</p>
                </div>
                <div className="p-6">
                  <BookingCalendar />
                </div>
        ...
        {/* Stats bar */}
        <div className="relative z-10 border-t border-slate-100 bg-slate-50 mt-8 lg:mt-0">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-md mx-auto">
              Pricing adapts to your home size and whether you have pets. Book above or call for a free, no-pressure quote.
            </p>
          </div>
          ...
          <div className="text-center mt-10">
            <a href="#booking" className="inline-flex flex-col items-center bg-sky-500 hover:bg-sky-600 text-white px-10 py-4 rounded-xl transition-all duration-200 cursor-pointer mcs-h shadow-sm hover:shadow-[0_4px_14px_rgba(14,165,233,0.4)] hover:scale-[1.02]">
              <span className="font-bold text-base">Book Free Cleaning</span>
              <span className="text-white/70 text-sm mt-0.5">Free · No Commitment</span>
            </a>
          </div>
        </div>
      </section>
      ...
      <section id="why-us" className="py-24 px-5 sm:px-8 bg-sky-300">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mt-10">
            <a href="#booking" className="inline-flex flex-col items-center bg-[#0A2540] hover:bg-[#0f3060] text-white px-10 py-4 rounded-xl transition-all duration-200 cursor-pointer mcs-h shadow-md hover:scale-[1.02]">
              <span className="font-bold text-base">Book Free Cleaning</span>
              <span className="text-white/55 text-sm mt-0.5">Free · No Commitment</span>
            </a>
          </div>
        </div>
      </section>
      ...
      <section id="areas" className="relative py-24 px-5 sm:px-8 bg-[#0A2540] overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="text-center">
            <a href="#booking" className="inline-flex flex-col items-center bg-sky-500 hover:bg-sky-600 text-white px-10 py-4 rounded-xl transition-all duration-200 cursor-pointer mcs-h shadow-md hover:shadow-[0_4px_20px_rgba(14,165,233,0.45)] hover:scale-[1.02]">
          ...
          <div className="grid lg:grid-cols-[1fr_400px] gap-12 items-center">
            ...
            <div>
              ...
              <p className="text-white/50 text-base leading-relaxed mb-8 max-w-sm">
                Book directly above or call us. We'll ask about your home size and pets, and give you a straight price — no surprises.
              </p>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-xl">
              ...
              <a href="#booking" className="w-full flex flex-col items-center bg-sky-500 hover:bg-sky-600 text-white py-4 rounded-xl transition-all duration-200 cursor-pointer mcs-h shadow-sm hover:shadow-[0_4px_18px_rgba(14,165,233,0.5)] hover:scale-[1.02]">
                <span className="font-bold text-base">Book Free Cleaning</span>
                <span className="text-white/65 text-sm mt-0.5">Free · No Commitment</span>
              </a>
