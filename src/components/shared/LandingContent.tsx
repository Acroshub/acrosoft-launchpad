    ...
    <div className="min-h-screen bg-background selection:bg-primary/10">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8 animate-in fade-in slide-in-from-left-8 duration-1000">
              ...
              <p className="text-xl text-muted-foreground max-w-lg leading-relaxed">{T.hero.p}</p>
              <div className="flex flex-wrap gap-4">
                <Button asChild size="lg" className="h-14 px-8 rounded-2xl font-black text-lg shadow-xl shadow-primary/25 hover:scale-105 transition-all group">
                  <a href="#agendar">{T.hero.cta1} <ArrowRight size={20} className="ml-2 group-hover:translate-x-1 transition-transform" /></a>
                </Button>
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className={`grid gap-8 max-w-6xl mx-auto ${services.length === 2 ? "md:grid-cols-2" : services.length >= 3 ? "md:grid-cols-3" : "md:grid-cols-1 max-w-sm"}`}>
            {services.map((svc) => {
              ...
              return (
                <div key={svc.id} className={`flex flex-col bg-background rounded-[32px] p-8 border-2 transition-all duration-500 hover:-translate-y-2 relative group ${popular ? "border-amber-400/60 shadow-xl shadow-amber-500/10" : "hover:border-primary/50"}`}>
                  ...
                  {popular && (
                    <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-amber-500 text-white text-xs font-bold shadow-lg shadow-amber-500/30">{T.plans.recommended}</Badge>
                  )}
                  <div className="space-y-2 mb-8">
                      ...
                      <div className="flex items-center gap-2 font-bold">
                        <Badge variant="secondary" className={popular ? "bg-amber-500/10 text-amber-600" : "bg-primary/10 text-primary"}>
                          {(svc.recurring_discount_pct ?? 0) > 0 ? (
                            <><span className="line-through opacity-60 mr-1">${svc.recurring_price.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>${Math.round(svc.recurring_price * (1 - (svc.recurring_discount_pct ?? 0) / 100)).toLocaleString()}</>
                    ...
                    {svc.description && <p className="text-sm text-muted-foreground pt-1">{svc.description}</p>}
                  </div>
                  ...
                  <div className="space-y-6 pt-6 border-t border-border/50">
                    ...
                    <Button asChild className={`w-full h-14 rounded-2xl font-black text-base transition-all hover:scale-[1.02] ${popular ? "bg-amber-500 hover:bg-amber-600 text-white shadow-lg shadow-amber-500/25" : ""}`} variant={popular ? "default" : "outline"}>
            ...
            {steps.map((step, i) => (
              <div key={step.title} className="text-center space-y-6 group">
                <div className="relative mx-auto w-24 h-24 rounded-[32px] bg-secondary flex items-center justify-center transition-all duration-500 group-hover:rotate-6 group-hover:scale-110 group-hover:bg-primary group-hover:shadow-xl group-hover:shadow-primary/30">
                  ...
                  <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-background border-2 border-primary flex items-center justify-center text-xs font-black">{i + 1}</div>
                </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-16 items-center max-w-6xl mx-auto">
            <div className="space-y-8">
              ...
              <p className="text-lg text-muted-foreground leading-relaxed">{T.why.p}</p>
              <div className="space-y-4">
                {[T.why.check1, T.why.check2].map((check) => (
                  <div key={check} className="flex items-center gap-3 p-4 bg-background rounded-2xl border border-border/50 shadow-sm">
      ...
      <section className="bg-primary py-24 md:py-40 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center space-y-10 relative">
          <h2 className="text-4xl md:text-6xl font-black text-white leading-tight max-w-4xl mx-auto">{T.cta.h2}</h2>
          <p className="text-xl text-white/80 max-w-2xl mx-auto font-medium">{T.cta.p}</p>
