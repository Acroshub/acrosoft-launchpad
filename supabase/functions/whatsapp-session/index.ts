...
Deno.serve(async (req) => {
  ...
  // Validate JWT — only the authenticated user can manage their own session
  ...
  // Use service role for direct DB reads (qr, status)
  ...
  switch (action) {
    ...
    // ── status: read connection status + phone number ─────────────────────────
    case "status": {
      const { data } = await adminDb
        .from("whatsapp_sessions")
    ...
    // ── disconnect: destroy session in Baileys + clean DB ─────────────────────
    case "disconnect": {
    ...
    // ── send: send a WhatsApp message (used by send-reminders) ────────────────
    case "send": {
